...
def send_likes(uid):
    likes_api_response = requests.get(f"https://nope.vercel.app/like?uid={uid}&server_name=ind")
...
def get_random_avatar():
	avatar_list = [
        '902000084', '902000306', '902000208', '902000209', '902000210', 
        '902000211', '902037031', '902049003', '902027022', '902000084', 
        '902000306', '902000208', '902000209', '902000210', '902000211','902037031','902049003','902027022','902037031','902049003','902027022','902037031','902049003'
    ]
...
[FFD700]TELEGRAM : [FFFF00]GAMIGO YT
[1E90FF]╚══════════════════════════╝""", idinv
                        )
                )
                senthi = False
...
[b][i][A5E2CFٍ]Instagram: @GAMIGO_YT[/b]

[b][c][1E90FF] Telegram: GAMIGO YT""",uid
                            )
                        )
                else:
                    pass  
...
If Anyone want to buy TCP bot or Buy Access

Just Contact Me...

[b][i][A5E2CFٍ]Telegram: GAMIGO YT[/b]

[b][c][1E90FF]Instagram : @GAMIGO_YT[FFFFFF]
...
[C][B][FF6347] Developer    —͟͞͞ GAMIGO YT </>""", uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"/x" in data:
                try:
                    ...
                    if len(command_split) > 1:
                        ...
                        json_result = get_available_room(data.hex()[10:3])
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["2"]["data"]
...
# -------------------- /like COMMAND --------------------
            if "1200" in data.hex()[0:4] and b"/like" in data:
                try:
                    print("Received /like command")
                    command_split = re.split("/like", str(data))

                    if len(command_split) > 1:
                        # Extract player_id (UID) from the command
                        player_id_match = re.search(r"(\d{5,})", command_split[1])
                        if player_id_match:
                            player_id = player_id_match.group(1).strip()
                        else:
                            json_result = get_available_room(data.hex()[10:])
                            parsed_data = json.loads(json_result)
                            uid = parsed_data["5"]["data"]["1"]["data"]
                            clients.send(
                                self.GenResponsMsg(
                                    "[FF0000]Usage: /like [uid]", uid
                                )
                            )
                            continue

                        print(f"Sending like to UID: {player_id}")
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"{generate_random_color()}Sending like to {fix_num(player_id)}...", uid
                            )
                        )

                        # API call
                        api_url = f"https://only-client-like.vercel.app//like?uid={player_id}&server_name=ind&api_key=123"
                        response = requests.get(api_url)
                        ...
                        if api_data.get("status") == 1:
                            ...
                            player_name = res.get("PlayerNickname", "Unknown")
                            before = res.get("LikesbeforeCommand", 0)
                            after = res.get("LikesafterCommand", 0)
                            level = res.get("Level", 0)
                            given = res.get("LikesGivenByAPI", 0)
                            remaining = res.get("RemainingRequests", "0/0")

                            response_message = (
                                f"[C][B][00FF00]✿——— LIKE SENT ——✿\n"
                                f"[B][FFA500]• Name: [FFFFFF]{player_name}\n"
                                f"[B][FFA500]• UID: [FFFFFF]{fix_num(player_id)}\n"
                                f"[B][FFA500]• Level: [FFFFFF]{level}\n"
                                f"[B][FFA500]• Likes Before: [FFFFFF]{fix_num(before)}\n"
                                f"[B][FFA500]• Likes After: [FFFFFF]{fix_num(after)}\n"
                                f"[B][FFA500]• Likes Given: [FFFFFF]{fix_num(given)}\n"
                                f"[B][FFA500]• Remaining API: [FFFFFF]{remaining}\n"
                                f"[C][B][00FF00]✿ ──────── ✿\n"
                            )

                        else:
                            response_message = f"[FF0000]Error: {api_data.get('message', 'Failed to send like.')}"
                    ...
                    clients.send(
                        self.GenResponsMsg(
                            "[FF0000]Error: Failed to send like via API.", uid
                        )
                    )
                except Exception as e:
                    ...
                    clients.send(
                        self.GenResponsMsg(
                            f"[FF0000]An error occurred: {e}", uid
                        )
                    )
# -------------------- /spm COMMAND --------------------            
            

            if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00]GAMIGO HERE [ffffff]BOT" in data:
            	pass
            else:
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
	            ...
	            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""[B][C][FFFF00] WELCOME TO [ffffff]GAMIGO GAME BOT  !! 

[C][B][00FF00]/🤔like [id] -> [C][B][FFFFFF]Apne Id Pe 100 Likes Le
...
[00FF00َ] /🤔ai [word] -> [C][B][FFFFFF]Chat GPT Se Swal Puchhe [ BHARAT AI ]
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3297286614", password="10322AE51EFB394641A2085E3A599978BD319BC774E712917H709F1E1C18CFD4")
        client_thread.start()
    except Exception as e:
